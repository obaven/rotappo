...
| `phenome-ui-terminal` | CLI formatting + command dispatch | domain, ui-presentation, adapter-primer | 
| `phenome-ui-tui` | TUI adapter (ratatui/crossterm) | domain, ports, application, ui-presentation, ui-core | 
| `phenome-adapter-primer` | Primer integration + port impls | domain, ports, primer | 
...
| `primer` (bin) | Primer CLI entrypoint | ui-terminal | 
| `tui` (bin) | TUI composition root | ui-tui, adapter-primer, application | 
...
- `primer` CLI must stay thin and defer to `phenome-ui-terminal`
...
| `lib/ui/phenome-ui-terminal/src/cli/*` | `phenome-ui-terminal` | Primer CLI clap surface. |
| `lib/ui/phenome-ui-tui/src/*` | `phenome-ui-tui` | Ratatui adapter and TUI logic. |
| `lib/adapters/phenome-adapter-primer/src/*` | `phenome-adapter-primer` | Port impls + primer mapping. |
| `lib/adapters/phenome-adapter-primer/src/controller/*` | `phenome-adapter-primer` | Primer command handlers. |
...
1) **PrimerBackend wiring**
   - Previous: `PrimerBackend` built `Runtime` inside adapters.
   - Now: adapter returns port set only; runtime built in bin crate.

2) **TUI App construction**
   - Previous: `phenome-ui-tui/app` depended on `PrimerBackend`.
...
- `PrimerBackend` exposes ports only; runtime construction happens in bins.
...
- [x] Migrate primer adapter into `phenome-adapter-primer`.
