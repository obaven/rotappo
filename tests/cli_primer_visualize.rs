#![cfg(all(feature = "cli", feature = "module-primer"))]

use std::path::{Path, PathBuf};

use phenome_adapter_primer::controller as adapter_controller;
use phenome_ui_terminal::cli::primer::{AssemblyAction, Commands};
...
#[tokio::test]
async fn primer_cli_assembly_visualize_smoke() -> anyhow::Result<()> {
    let assembly_path = fixture_assembly_path();
    let output_path = std::env::temp_dir().join(format!(
        "phenome-assembly-visualize-{}.dot",
        std::process::id()
    ));
