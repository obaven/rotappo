...
## Macro Inventory
TUI adapter macros (owner: `rotappo-ui-tui`):
...
Source of record: `lib/ui/rotappo-ui-tui/src/macros/layout.rs`.

## Helper Inventory + Duplication Hotspots
- `centered_rect` existed in `bootstrap/utils.rs` and `util/rect.rs`. It now
  lives in `lib/ui/rotappo-ui-tui/src/util/rect.rs`, and bootstrap panels
  consume the shared helper.
- No core-layer macros are defined today, and no other helper duplication
  hotspots were identified in `lib/ui/rotappo-ui-tui/src/util`.

## Usage Guidance
- Prefer `rotappo_ui_tui::grid_spec!` or `crate::grid_spec!` for layout grids.
- Use `rotappo_ui_tui::util` helpers for Ratatui geometry and rendering glue.
