...
#[command(about = "Bootstrappo - Declarative Kubernetes Platform Bootstrap")]
...
For more info: bootstrappo <command> --help
"#)]
pub struct Cli {
    /// Path to the Bootstrappo config file
    #[arg(
        long = "config",
        alias = "config-path",
...
pub async fn run() -> anyhow::Result<()> {
    ...
    unsafe {
        std::env::set_var("BOOTSTRAPPO_CONFIG_PATH", &cli.config_path);
    }
